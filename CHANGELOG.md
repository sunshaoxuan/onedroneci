...
- HTTPS / 443 启用时将 HTTP 跳转监听端口固定为 80，避免页面传入 443 或其他端口后生成错误的重定向监听。
- 保留 `nginx_https.conf` 的两个完整 `server` 块，仅通过生成参数修正监听端口，并增加 HTTP 80 跳转与 HTTPS 443 SSL 的构建后校验。
...
- 標準版的客户机构名移动到构造类型之后，在机构封包和客户化模式中始终显示，并在客户化模式中设为必填。
- 客户化包未选择 `conf_prod` 时继续保留客户机构名，用于构建记录和成果物目录命名；标准发版继续使用“共通”。
- 构造类型切换只改变客户机构名的显示和校验状态，不再改写用户已经输入的客户机构名。

## 0.4.0 - 2026-07-14

### Added

- 標準版新增 `顧客化` 构造类型，可独立选择后端 `package.zip`、前端本体、Help、`conf_prod`、SQL 资材、数据连携、导入计划和运行环境资材。
- 构建终端支持 Help-only、`conf_prod`-only 等无前端本体的 `web.zip` 组装，前端分支只在选择前端本体时必填。

### Changed

- 顾客化资材选择列表放在参数区顶部，未勾选资材对应的分支、Revision、环境信息、导入计划和中间件设置块会立即隐藏并停止提交。
- 顾客化最终交付严格按勾选项组装。运行环境模板中的旧 `package.zip`、`web.zip` 会先移除，仅重新植入本次选中的代码资材。
- 顾客化允许仅执行宿主机资材组装，未选择后端、前端本体、Help 或 `conf_prod` 时不向构建终端派发代码构建。

## 0.3.74 - 2026-07-10

### Fixed

- `run_all_sql.ps1` を再帰的な SQL 検出へ変更し、既知ファイルの増減があっても現在の包内 SQL から実行計画を構築するようにした。
- 実行前に全 SQL が重複なく一度だけ計画へ入っていることを検証し、漏れ・重複がある場合は DB 接続前に終了するようにした。
- ForeignTable の前後分割を現在件数ではなく既定順序番号で判定し、ファイル数の変動で後半 SQL が前倒しされないようにした。
- `ContinueOnError` 指定時も SQL 失敗件数を保持し、最後の終了コードを成功にしないようにした。

## 0.3.73 - 2026-07-10

### Fixed

- データ連携配下の `all.sql` をコピー・自動生成しないようにし、`run_all_sql.ps1` を同領域の唯一の一括実行入口にした。

## 0.3.72 - 2026-07-10

### Added

- 標準版の機関封包で `データ連携/run_all_sql.ps1` を生成するようにした。画面の DB 主機情報を OHR/tenant 共通設定、UPDS サービス情報を UPDS 設定として反映する。
- SQL 一括実行スクリプトは各サブフォルダーの `all.sql` を対象外とし、個別 SQL を定義済み順序で実行する。

### Security

- 参考スクリプト内の顧客固有ホスト・パスワードをテンプレートへ含めず、機関封包時だけ当該ジョブの設定値を生成物へ反映する。

## 0.3.71 - 2026-07-10

### Fixed

- 製品バージョンまたは標準版構造種別の切替時に、資材番号から自動設定されたバックエンドブランチ、フロントエンドブランチ、Help SVN Revision も同時にクリアするようにした。

## 0.3.70 - 2026-07-10

### Fixed

- 製品バージョンまたは標準版構造種別を切り替えた際、資材番号と候補フィルターを即時クリアするようにした。
- 画面切替前に開始した資材候補・分岐自動取得の応答が、切替後の画面へ古い値を書き戻さないようリクエスト順序を検証するようにした。
...
- 標準版の導入計画を参考 Excel「１．１．導入プラン」に合わせ、年末調整の個人ポータル側と年末調整側を個別に設定できるようにした。
- 参考 Excel にない住宅利用申請系の表示を削除し、住所の印字設定と顔写真管理の表示名を修正した。

## 0.3.65 - 2026-07-02

### Added

- 主控台成果物区新增整包下载功能，按交付目录生成临时 zip，默认 7 天有效；过期后自动删除下载包并允许从保留的交付目录重新打包。

## 0.3.64 - 2026-06-30

### Changed

- 主控台构造参数页面为各功能块增加低饱和浅底色背景，强化区块边界，降低全黑白表单的阅读负担。
...
- 標準版 `标准发版` 不再强制跳过 Help 构造；`build_help` 改为由画面“生成 Help 包及相关资源”勾选项决定，Help SVN revision 也在标准版两种构造类型下可输入。

## 0.3.61 - 2026-06-26

### Fixed

- 標準版 `标准发版` 恢复显示并校验资材番号，与后端分支、前端分支共用同一行基础构造参数；只隐藏下方详细参数与导入计划。

## 0.3.60 - 2026-06-26

### Added

- 標準版新增构造类型选择：`标准发版` 只构造并输出 `package.zip` 与 `web.zip`，`机构封包` 保持现有完整交付包流程。
- 标准发版输出目录固定为 `dist\standalone\標準発版 <主控タスクID>\`，不要求客户环境、SQL、数据连携或中间件封包参数。

## 0.3.59 - 2026-06-24

### Fixed

- 主控台选中历史任务时，成果物信息区只由任务详情接口渲染，避免定时刷新时在轻量任务数据和完整成果物数据之间反复重绘。

## 0.3.58 - 2026-06-24

### Fixed

- 主控台中间件版本说明改为短提示文案，并调整为低对比轻量提示样式，避免在参数面板中形成大段正文。

## 0.3.57 - 2026-06-23

### Changed

- NHO版交付目录名前缀固定为 `NHO`，目录格式为 `NHO <主控タスクID>`。

## 0.3.56 - 2026-06-23

### Changed

- 標準版交付目录命名改为 `<顧客機関名> <主控タスクID>`，便于在输出目录中按客户查找构造结果。
- 历史任务读取时会迁移仍存在的旧目录，并同步 `metadata.json` 中的成果物路径。

## 0.3.55 - 2026-06-23

### Added

- 非内置下载版 nginx/Redis 中间件包生成时自动合并 `addons/<product>/` 下的补充文件，确保下载源缺少安装脚本或配置文件时仍可被安装工具使用。

### Changed

- 中间件缓存包会检查 addon 文件是否存在且内容一致；缓存缺失或内容不一致时自动重建。
...
- NHO 前端 Direct 构建产物 `web.zip` 恢复 `ohr-cicd/web_prod/` 两层目录结构，避免升级程序按固定代码資材路径查找时无法命中前端文件。
...
- Direct 构建开始时清理同产品版本下旧构建产物、后端 `package` 输出和前端 `release_*` 输出，保留源码工作区和依赖缓存；標準版前端最终打包临时目录改到本次共享产物目录下，减少系统 `/tmp` 空间压力。
- 主控台构建终端状态卡新增 CPU 数、空闲内存和空闲硬盘大小；平时每 5 分钟刷新，构建开始后每 10 秒刷新。
