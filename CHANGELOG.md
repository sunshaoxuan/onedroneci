...
- 標準版 `2.ohr/99.import_plan.sql` 的菜单映射扩展为参考库中当前画面可对应的完整 `ohr_menu` 菜单码；多个画面项共用同一菜单码时，仅在相关项全部关闭后生成关闭 SQL。

## 0.3.36 - 2026-05-25

### Added

- 標準版交付包新增 `2.ohr/99.import_plan.sql`，根据画面公開計画生成 `ohr_menu` 非公開更新和相关定时任务暂停更新。

## 0.3.35 - 2026-05-25

### Added

- 標準版交付包新增 `1.tenant/99.import_plan.sql`，根据画面公開計画、邮件利用、駅すぱあと利用、係・講座利用生成 tenant 导入设置 SQL。
