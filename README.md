...
4. `標準版` 可选择构造类型：
   ...
   - `客户化`：保留机构封包参数。客户机构名固定显示在构造类型之后，并在客户化模式设为必填，再通过顶部资材列表独立选择后端、前端本体、Help、`conf_prod`、SQL 资材、数据连携、导入计划和运行环境资材。未勾选资材的设置块会隐藏，构建请求、`web.zip` 和最终交付目录都排除对应内容。
5. `標準版` 前端 Direct 构建中：
...
13. 標準版完整交付包可为 nginx、Redis、MinIO 选择中间件版本。nginx、Redis 默认使用模板内置包；MinIO 默认不包含，勾选后使用模板内置包或所选版本。选择其他版本时，主控台从官方发布源下载到宿主机缓存，并在重建 `OneHrStandalone.zip` 时替换 `OneHrStandalone/software/` 下的同名 zip。Azure Blob Storage 默认关闭，最终包中的 Azure 代理段按开关统一注释或取消注释。
14. HTTPS / 443 启用时，HTTP 入口固定监听 80 并返回到 HTTPS 443 的 301 跳转，HTTPS 业务块继续保留生成器产生的 TLS、代理与静态资源配置。
...
- 標準版客户化模式在参数区顶部显示八类资材勾选项。支持后端单包、前端单包、Help 单包以及宿主机资材的任意有效组合；至少选择一项。
