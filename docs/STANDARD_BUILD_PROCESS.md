...
本文说明“庶务事务 標準版”的 Direct 构造过程。標準版支持三种构造类型：`标准发版`、`机构封包`、`客户化`。标准发版输出代码发布资材；机构封包生成完整客户交付目录；客户化按用户勾选的资材自由组装。
...
- 产品版本：`標準版`
- 构造类型：`标准发版`、`机构封包` 或 `客户化`
- 资材编号：标准发版与机构封包均必填。机构封包会写入最终 `version.txt` 第一行 `資材:<资材编号>`。候选值来自构建终端读取的 `お客様環境` SVN，目录名 `資材-YYYYMMDD` 或 `資材_YYYYMMDD` 会被识别为候选。
...
客户化模式在参数区顶部显示以下资材，默认全部勾选，至少保留一项：
...
勾选项同时决定设置块是否显示。后端与前端分别控制各自分支；Help 控制 SVN revision；运行环境资材控制中间件版本；导入计划控制服务信息和画面公开计划。AP、DB、WEB 等共享设置块会在任一依赖资材启用时显示。

客户化最终目录严格按选择结果生成：

- 后端、前端本体、Help、`conf_prod` 中任一项启用时，构建终端只组装对应代码内容。Help-only 会生成只包含 `ohr-cicd/web_prod/help/` 的 `web.zip`。
- SQL、数据连携、导入计划和运行环境资材由宿主机分别组装，未启用的目录不创建。
- 运行环境资材启用时，模板内旧 `package.zip` 和 `web.zip` 会先删除，随后只写入本次选择的代码包。
- 如果没有选择任何需要构建终端处理的代码资材，主控台直接进行宿主机资材组装。
...
画面公開計画的树形结构按导入指南 Excel `１．１．導入プラン` 维护。年末調整下的 `個人ポータル` 和 `年末調整` 是两组可分别设定的二级分类，`年末調整`、`税法扶養申請` 等同名画面不会共用同一个页面勾选项。参考 Excel 中不存在的住宅利用申請系项目不在主控台显示，`住所の印字設定` 与 `顔写真管理` 按参考 Excel 名称显示。
