...
標準版机构封包同时生成 `データ連携/run_all_sql.ps1`。该脚本以 OHR 和 UPDS 两组连接参数替代参考脚本中的重复参数，OHR 参数复用于 `ohr`、`tenant`、`djn_self`。数据连携子目录不复制也不生成 `all.sql`，交付脚本自身负责个别 SQL 的执行顺序。脚本模板不保存客户固有连接值，最终交付脚本由本次任务参数渲染。
