...
宿主机网站固定监听 `0.0.0.0:8091`，页面上只称呼远端为“构建终端”。庶务事务系统分为 `標準版` 与 `NHO版`：標準版可选择 `标准发版`、`机构封包` 或 `客户化`，NHO版生成代码共通包 `共通.zip`。固定包、SQL 模板和产品交付输出目录都保留在宿主机侧。
...
標準版构造类型为 `客户化` 时，参数区顶部提供八类资材勾选项。勾选资材后显示对应设置块，取消后立即隐藏并禁用。客户化可以只生成后端、只生成前端本体、只生成 Help，也可以组合 SQL、数据连携、导入计划和运行环境资材。主控台在提交请求和二次打包时使用同一组选项，保证未勾选内容不会进入交付目录或模板 ZIP。

产品版本或標準版构造类型发生切换时，主控台会清空资材番号、后台分支、前台分支、Help SVN Revision 及候选过滤状态。切换前已经发送的资材候选和分支查询即使稍后返回，也不会覆盖当前页面。
...
WEB 主机信息启用“HTTPS / 443 設定を生成”时，页面将 HTTP 端口固定为 80，主控台和构建终端也会在服务端再次归一为 80。最终 nginx 配置保留生成器的两个完整 `server` 块，第一个块负责 80 到 443 的 301 跳转，第二个块负责 443 SSL 业务处理。
...
`run_all_sql.ps1` は画面の DB 主機情報と UPDS サービス情報から生成され、六つの業務ディレクトリを再帰検索して個別 SQL を順序実行します。各サブフォルダーの `all.sql` は生成されません。実行前に実ファイルと実行計画が一対一であることを検証し、SQL エラー時は非ゼロで終了します。
